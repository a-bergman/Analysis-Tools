...
def scatterplots(columns, y, df, titles, labels, ylabel, ticks, row, col, mark, ci = None):
    count = 0
    fig   = plt.figure(figsize = (14,7), facecolor = "white")
    for c, column in enumerate(columns):
        count += 1
...
def heatmap(df, title, vmin, vmax, cmap = "RdBu"):
    ...
    with sns.axes_style("white"):
        sns.heatmap(corr, cmap = cmap, mask = mask, vmin = vmin, vmax = vmax, annot = True)
    plt.xticks(size = 14)
    plt.yticks(size = 14);
